...
from mp2.common.mpi_data_utils import sync_weights, average_grads, mpi_gather_objects, mpi_sum, \
...
class MAML(object):
    ...
    def __init__(self, config):
        ...
        self.v_network = None                  # value network
        self.initial_v_weights = None          # initial value network weights
        ...
        self.v_optimizer = None                # value optimizer
        ...
        #self.total_episodes = 0  # todo: rm
        #self.config['render'] = True
        #self.config['render_folder'] = '/Users/markojj1/Desktop'
    ...
    def process_config(self):
        ...
        self.config.setdefault('experiences_per_task', -1)     # will allow us to base updates on # experiences
        self.config.setdefault('adaptation_steps', 1)
        self.config.setdefault('max_ep_length', -1)
        ...
        self.config.setdefault('gradient_surgery', False)
        self.config.setdefault('test_tasks', 100)  # testing runs (test_tasks * trajectories_per_task) trajectories
        self.config.setdefault('use_test_set', False)
        self.config.setdefault('sample_in_testing', False)
        self.config.setdefault('test_output_file', 'test_results')
    ...
    def train(self):
        ...
        initial_steps, last_checkpoint, theta_pi = self.initialize_networks()
        ...
        if self.config['experiences_per_task'] <= 0:
            ...
            while True:
                ...
                if self.buffer.observations.shape[0] >= self.experiences_this_task:
                    break
        # summaries = self.concatenate_dict_of_lists(summaries)
        summaries = self.mean_of_dict_of_lists(self.concatenate_dict_of_lists(summaries))  # todo: move average out
        return summaries
    ...
    def compute_max_entropy_term(self, trajectory_buffer):
        ...
        if self.config['max_entropy'] > 0:  # takes control bounds into account
            ...
            if self.config['environment']['normalize_reward'] > 0:
                max_entropy_term *= self.config['environment']['normalize_reward']
        else:
    ...
    def update_value_network(self):
        ...
        """
        self.v_network = LinearFeatureBaseline(self.config['v_network'])
        self.collect_buffers()  # todo: need
        if self.group_id == 0:
            self.v_network.fit(self.buffer)
        self.group_comm.Bcast(self.v_network.coeffs, root=0)

    def update_experiences(self):
        """  Update experiences using new value network  """
        self.buffer.values = self.v_network.predict(self.buffer.observations)
    ...
    def compute_meta_gradient(self, current_params):
        ...
        if self.group_id == 0:
            # print(str(self.id) + ' : ' + 'computing gradient')
            # sys.stdout.flush()
            self.update_meta_buffer()  # for keeping the meta-update within some distance of the previous meta-policy
            for j in range(self.config['adaptation_steps']):
                self.buffer = self.buffers[j]
                self.inner_step(set_grad=True)  # current params doesn't change
            ...
            pi_loss = torch.mean(pi_loss, dim=0)
            update_module_params(self.pi_network, current_params)  # puts grad_fn to none, restoring leaf node
    ...
    def initialize_networks(self):
        ...
        if self.config['use_prior_nets']:
            ...
            initial_v = torch.load(os.path.join(self.config['model_folder'], 'value-initial.pt'))
            self.v_network.load_state_dict(initial_v['model'])
        else:
            total_steps, last_checkpoint = 0, 0
            if self.id == 0:
                torch.save({'model': self.v_network.state_dict()},
                           os.path.join(self.config['model_folder'], 'value-initial.pt'))
        sync_weights(MPI.COMM_WORLD, self.pi_network.parameters())
        ...
        self.initial_v_weights = deepcopy(self.v_network.state_dict())
