...
from mp2.common.mpi_data_utils import mpi_gather_objects, mpi_sum, mpi_statistics_scalar
from mp2.base.rl.policy_optimizer import PolicyOptimizer
from mp2.risk.dist.utility_functions import load_utility_function
from mp2.risk.dist.weight_coefficients import load_weight_coefficient_function
from mp2.risk.dist.metrics import load_metrics
...
class CDFPolicyOptimizer(PolicyOptimizer):
    ...
    def process_config(self):
        ...
        self.config['utility'].setdefault('variable_ref', False)   # Whether utility has a variable reference point
    ...
    def process_trajectory(self, trajectory_buffer, trajectory_info):
        """  Processes a completed trajectory, storing required data in buffer  """
        if self.config['utility']['variable_ref']:  # compute later, once we have rewards for whole batch
            utilities, q_values = None, None
        else:
            utilities = self.compute_utilities(trajectory_buffer[:, 2], trajectory_info)
            q_values = self.compute_target_values(utilities) if self.mode == 'train' else None
        self.buffer.update(trajectory_buffer, q_values, utilities=utilities)
    ...
    def compute_policy_loss(self, observations, actions, advantages, old_log_probs, old_policies, mb=None, clip=True):
        """  Compute policy loss, entropy, kld  """
        pi_loss, entropy, kld = super().compute_policy_loss(observations, actions, advantages,
                                                            old_log_probs, old_policies, mb, clip)
