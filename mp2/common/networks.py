...
class GaussianMLP(MLP):
    ...
    def process_config(self):
        ...
        self.config.setdefault('min_log_std', None)  # -20 for safety-starter-agents
        self.config.setdefault('max_log_std', None)  # 2 for safety-starter-agents
...
class BetaMLP(MLP):
    ...
    def process_config(self):
        ...
        super().process_config()
        self.config['discrete'] = 0
        self.config.setdefault('max_ab', 15)

    def forward(self, x):
        """  Run forward pass of network  """
        alpha = torch.nn.functional.relu(self.alpha_layer(self.mlp(x)))
        alpha = (alpha + torch.ones(alpha.size())).clamp(min=1.0, max=self.config['max_ab'])
        beta = torch.nn.functional.relu(self.beta_layer(self.mlp(x)))
        beta = (beta + torch.ones(beta.size())).clamp(min=1.0, max=self.config['max_ab'])
        return alpha, beta
