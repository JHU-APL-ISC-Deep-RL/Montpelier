...
'''  This is a (mostly) stable-baselines module for pre-processing Atari environments  '''
...
class FrameStack(gym.Wrapper):
    def __init__(self, env, n_frames):
        ...
        self.observation_space = spaces.Box(low=0, high=255, shape=(1, shp[0], shp[1], shp[2] * n_frames),
                                            dtype=env.observation_space.dtype)
    ...
    def _get_ob(self):
        ...
        return np.expand_dims(LazyFrames(list(self.frames))[:], axis=0)
...
def wrap_deepmind(env, episode_life=True, clip_rewards=False, frame_stack=True, scale=True, warp=True, **kwargs):
    """
    ...
    :param warp: (bool) whether or not warp the frame (grayscale, reshape)
    :return: (Gym Environment) the wrapped atari environment
    """
    if episode_life:
        env = EpisodicLifeEnv(env)
    if "get_action_meanings" in dir(env.unwrapped) and 'FIRE' in env.unwrapped.get_action_meanings():
